...
fs.readFile(buildFilePath, "utf8", (err, data) => {
  ...
  // Modify the src attributes by replacing /sth with /build/sth
  const modifiedDataWithSrc = modifiedData.replace(
    /(src=")\/(.*?")/g,
    "$1/build/$2"
  );

  fs.writeFile(buildFilePath, modifiedDataWithSrc, "utf8", (err) => {
    if (err) {
